 ...
 * This is an example implementation built from scratch and used for understanding 
 * Linked List data structure requirements.
 * The standard library implementation of LinkedList is more advanced and performant..
...
impl<T> DoublyLinkedList<T> {
    ...
    pub fn push_front(&mut self, data: T) {
        ...
        if self.head.is_none() {
            ...
            // Update the old head's prev to point to the new node.
            unsafe {
                (*raw_new_node).next.as_mut().unwrap().as_mut().prev = Some(Box::from_raw(raw_new_node));
            }
    ...
    unsafe fn push_front_node(&mut self, node: Box<Node<T>>) {
        ...
        unsafe {
            ...
            (*raw_node).prev = None;
            let node = Some(Box::from_raw(raw_node));

            match self.head {
                None => self.tail = node,
                // Not creating new mutable (unique!) references overlapping `element`.
                Some(head) => (*Box::into_raw(head)).prev = node,
            }
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        // Fix: This does not check for underflow.
...
pub struct IterMut<'a, T> {
    ...
    marker: std::marker::PhantomData<&'a mut Node<T>>,
}

pub struct IterRevMut<'a, T> {
    next: Option<NonNull<Node<T>>>,
    marker: std::marker::PhantomData<&'a mut Node<T>>,
}
...
impl<T> DoublyLinkedList<T> {
    pub fn iter_mut(&mut self) -> IterMut<T> {
        IterMut {
            next: self.head.as_deref_mut().map(NonNull::from),
    ...
    pub fn iter_rev_mut(&mut self) -> IterRevMut<T> {
        IterRevMut {
            next: self.tail.as_deref_mut().map(NonNull::from),
            marker: std::marker::PhantomData,
        }
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        unsafe {
            self.next.map(|mut node| {
                self.next = node.as_mut().next.as_deref_mut().map(NonNull::from);
...
impl<'a, T> Iterator for IterRevMut<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        unsafe {
            self.next.map(|mut node| {
                self.next = node.as_mut().prev.as_deref_mut().map(NonNull::from);
                &mut node.as_mut().data
            })
        }
    }
